...
- committed `oes-core-vlen-time.oes.h5`: 3 rows, 5 columns, final header
  `900.0000000001`, 전체 선택 3x5 clipboard와 final value `203` PASS
- external `sample_current_rules.oes.h5`: 128 rows, 65 columns, final header
  `900.0000000001`, 64+1 projection 전체 선택 128x65 clipboard와 final value `24971` PASS
- query search control count 0 PASS

실제 파일 증거 screenshot은 `native-oes-real-full-copy.png`이며 copy status는 `Copied 128 rows.`를
표시한다.
