...
- query logical columns와 현재 page projection 분리, foreground/prefetch request scheduling
- null, 긴 문자열, 긴 컬럼 이름, nested 값 표시
- raw typed value와 타입별 display/copy formatter, settings migration parity
- timestamp 정밀도/timezone metadata와 timezone 없는 기본 표시·복사
...
## 2. Playwright 브라우저 상호작용 검증

Tauri API를 `browserMockBackend`로 대체한 로컬 프런트엔드를 Playwright Chromium에서 실행한다.
환경 구성, 명령, locator와 산출물 규칙은 `docs/PLAYWRIGHT.md`를 따른다. UI 변경의 기본 자동
관문은 `npm run test:e2e`이며 `playwright.config.ts`의 세 viewport project를 모두 통과해야 한다.
개발 중에는 영향 spec이나 project만 실행할 수 있지만 완료 전에는 전체 E2E를 한 번 실행한다.
...
- 열 header separator 더블클릭과 column menu의 너비 자동 맞춤
- `Shift+화살표`, `Ctrl+화살표`, `Ctrl+Shift+화살표`
- `Ctrl+Alt+화살표`, `Ctrl+Alt+Shift+화살표`
- `Home`, `End`, `PageUp`, `PageDown`, `Ctrl+A`, `Escape`
- 정렬 결과의 first/middle/last에서 연속 `PageDown`/`PageUp`과 foreground 우선 page loading
- 선택 범위 자동 scroll과 가상화 이후 선택 유지
- 5,850,000행과 10,000,000행 segmented scroll의 segment 경계와 실제 마지막 행 이동
- 실제 LF/CRLF, literal `\n`, 최대 2줄 string clamp와 전체 값 보기
- 타입별 전역 display 설정 변경과 기본/displayed/raw copy 구분
- input focus 중 grid shortcut 차단
- clipboard에 기록되는 TSV 확인
- drag 진입·이탈 중 drop target 표시
- `Ctrl+F` Find open/focus, typing 중 backend call 0과 `조회`/Enter 명시적 실행
- filter/sort 뒤 logical active cell 보존·clamp와 rectangular range의 단일 cell 축소
- 파일 tab과 column header의 pointer/keyboard reorder, drop indicator와 ID 기반 상태 보존
- partial/whole query copy의 filtered row, sorted order와 visible reordered column
- copy current operation/history, progress, cancel, Retry와 typed failure reason
- cache-valid tab 복귀의 page IPC 0과 blur·blank·loading flash 없음

각 동작은 시작 상태, 입력, 기대 결과, 실제 결과를 기록한다. 클릭이 성공했다는 사실만으로
PASS 처리하지 말고 논리 좌표, 화면 상태, scroll 위치 또는 clipboard 값을 assertion으로
확인한다. 브라우저 mock이 실제 Tauri 동작을 대신하지 않도록 native 전용 항목을 분리한다.

## 3. DOM geometry 검증

Playwright의 `locator.evaluate`로 `getBoundingClientRect`, computed style, `scrollWidth`, `clientWidth`,
...
- query foreground page가 pending→populated로 바뀌어도 row/header/selection geometry가 움직이지 않는다.
- 정렬 뒤 PageUp/PageDown과 Ctrl target의 active cell이 viewport 내부에 완전히 표시된다.
- 논리 row 수와 무관하게 physical `scrollHeight`가 문서화된 WebView 안전 상한 안에 있다.
- segment recenter 전후 첫 visible logical row, active cell과 selection outline이 일치한다.
- 실제 마지막 data row의 content와 border가 horizontal scrollbar와 겹치지 않고 완전히 보인다.
- tab 복귀 첫 paint에서 저장된 scroll·segment·active cell과 column projection이 일치한다.
- Find bar, multi-sort panel, drag indicator와 copy history가 minimum viewport에서 grid를 가리거나 잘리지 않는다.
- column reorder 뒤 header/cell/selection outline이 같은 visual column 경계에 정렬된다.
- multiline data row는 모두 같은 고정 높이이며 실제 문자열은 최대 2줄까지만 표시된다.
- auto-fit width는 header/cached display의 측정 결과와 1 CSS px 이내이고 80..800 px 범위다.
- auto-fit 뒤 인접 header/cell 정렬, horizontal scroll과 selection outline이 깨지지 않는다.
...
- 긴 값과 긴 컬럼 이름
- 2줄 multiline string과 timestamp raw/display/copy 상세
- 대용량 grid의 실제 마지막 행
- 타입별 전역 Display formats 설정
- 정렬 후 foreground page loading, populated와 재시도 가능한 page error
- 짧은 값, 긴 값과 multiline 값의 column auto-fit 전후
- Schema와 Metadata 화면
- `Ctrl+F` Find draft/committed와 previous/next 상태
- multi-sort priority panel, file/column drag와 keyboard reorder 결과
- query copy progress, current failure와 previous attempt history
- 대용량 tab을 떠났다가 cache로 복귀한 첫 populated frame

Playwright 성공 screenshot은 HTML report attachment로 남기고, Quality Agent는 이를 이미지
입력으로 열어 다음 항목을 독립 검토한다.
...
- WebView 최대 scroll 범위, segmented recenter와 native scrollbar geometry
- 실제 대용량 Parquet 정렬 후 query page latency, pending 해소와 keyboard focus
- 실제 5.85M filtered/sorted 1-column backend copy와 Windows clipboard atomic commit
- wide H5 adaptive copy, typed compression error와 H5 구조 판별
- 20회 document tab 왕복의 zero-reload geometry/focus restore
- installer로 설치된 앱
...
- `playwright-report/`와 `test-results/`는 로컬 생성물이므로 최종 Phase 증거로 직접 커밋하지
  않는다. 검토를 마친 최종 결과만 위 artifact 경로에 기록한다.
...
- separator 더블클릭과 keyboard-accessible menu auto-fit이 같은 결과를 내고 backend scan을 만들지 않는다.
