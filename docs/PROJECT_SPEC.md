...
`Auto`는 제한된 표본으로 타입을 자동 결정한 뒤 사용자 확인 modal 없이 즉시 준비를 계속한다.
애매한 열은 값 손실이나 잘못된 시간 해석보다 `Text`를 우선한다. 사용자가 명시적으로
`Ask Every Time`을 선택했거나 CSV Parsing Profile을 열었을 때만 타입 확인·수정 UI를 표시한다.
