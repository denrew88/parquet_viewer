...
이 문서는 CSV, Parquet와 Phase 11의 OEF H5 v3 확장을 포함한 데스크톱 뷰어의 상세 제품
요구사항과 기술 계약을 정의한다.
...
백엔드 경계 탐색기가 수행한다. 프런트엔드는 중간 page를 순차 요청하지 않고 반환된 좌표가 든
page만 cache miss일 때 최대 1회 요청한다.
...
Parquet query는 가능한 predicate와 projection을 scan에 push down한다. query result는 모든 source
column의 display/raw 문자열 복사본을 저장하지 않고 source row identity, result position과 필요한
typed predicate/sort 값으로 제한한다. page를 읽을 때 요청 projection만 bounded하게 materialize하고
안정 정렬의 최종 tie-breaker는 원본 row identity를 사용한다.
...
Phase 11은 범용 HDF5 browser가 아니라 다음 고정 구조만 지원한다.

```text
root attribute: format = "oefh5"
...
- `.h5`, `.hdf5`는 모든 open 진입점의 후보이며 signature, format/version과 구조를 최종 검증한다.
- Windows `.h5/.hdf5` file association은 모든 HDF5를 선점하므로 별도 승인 없이 추가하지 않는다.
- 64열을 넘는 grid와 copy는 기존 bounded projection 분할 계약을 유지한다.
