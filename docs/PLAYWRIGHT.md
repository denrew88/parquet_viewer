...
Phase 12 query page 테스트는 logical result columns와 현재 page projection을 별도로 mock한다. 정렬
완료 뒤 first page, middle/last jump, 빠른 wheel/scrollbar와 20회 연속 PageDown/PageUp을 실행하고
foreground 요청이 adjacent prefetch보다 먼저 발생하는지, 같은 offset/projection IPC가 합쳐지는지,
stale projection 응답과 loading key가 폐기되는지를 assertion한다. `Ctrl`, `Ctrl+Shift`, `Ctrl+Alt`,
`Ctrl+Alt+Shift`, Home/End와 PageUp/PageDown은 source row identity가 아니라 query logical position,
anchor, active cell, selection rect와 focus visibility로 검증한다. permanent `pending` 판정은 고정 sleep이
아니라 in-flight request, accessible status와 populated cell의 최종 상태를 사용한다. 브라우저 mock의
latency는 backend release 성능 증거를 대신하지 않는다.

Phase 12 query UX는 전역 match-only Filter control이 없는지와 header typed filter가 남는지 확인한다.
`Ctrl+F` 뒤 Find input focus, typing/debounce 중 backend call 0, `조회`/Enter의 단일 실행, Esc와
previous/next를 실제 keyboard로 검증한다. filter/sort commit 전후 active logical row와 column ID,
clamp된 target, 단일 cell로 축소된 range와 target page commit 뒤 focus visibility를 assertion한다.
filter→sort와 sort→filter는 같은 최종 mock plan에서 같은 ordered identity를 반환해야 한다.

파일 tab과 column header는 pointer drag와 keyboard reorder를 모두 실행한다. tab reorder/복귀는
document/session/query identity, scroll, segmented anchor, page/projection IPC count와 first paint 상태를
검사한다. cache-valid 5.85M tab을 다른 파일과 20회 왕복할 때 page IPC, blank/blur/loading flash가 없어야
한다. column reorder는 ID 순서, width, visibility, filter/sort priority, active cell, selection outline과
copy column order를 검사한다. multi-sort panel의 priority/direction/remove/reorder는 Shift+header와
동일한 plan을 만들어야 한다.

query copy browser test는 partial/whole selection의 filtered row, sorted order와 visible reordered
column을 clipboard mock으로 검증한다. 5.85M행 전체 cell을 browser에 만들지 않고 backend copy task의
progress/cancel/current/history event를 mock한다. current attempt와 이전 attempt ID·시각, typed failure
reason과 stale query 차단을 assertion한다. 실제 streaming 성능, H5 adaptive hyperslab과 Windows
clipboard atomic commit은 native/release 증거로 분리한다.
