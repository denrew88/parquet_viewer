...
4. `docs/PLAYWRIGHT.md`: Playwright 환경, 실행 명령, 테스트 작성과 증거 규칙
...
- Rust와 TypeScript가 같은 DTO 유효성 규칙을 각각 검사하면 경계 조합 parity test를 둔다.
...
토큰과 반복 실행은 `docs/AI_WORK_POLICY.md`에 따라 줄인다. `rg`와 필요한 줄 범위만 읽고,
실패 로그는 핵심만 수집한다. 구현 중에는 관련 테스트를 사용하고 전체 suite와 release/NSIS
build, 최종 GUI capture는 코드 확정 뒤 각각 한 번 실행한다. 단일 소유 작업에는 subagent를
추가하지 않으며 리뷰 지적은 모아서 한 번에 수정·재검토한다. 이 정책으로 테스트 기준,
보안 검증, 데이터 무결성 검증을 낮추지 않는다.
...
UI 변경은 `docs/UI_VALIDATION.md`에 정의된 Playwright interaction, DOM geometry, 세 가지
viewport screenshot, 실제 Tauri 검증을 적용한다. 공통 UI는 `npm run test:e2e`를 통과해야
하며 브라우저 검증만으로 native UI 항목을 PASS 처리하지 않는다.

백엔드 동작을 바꾸는 설정 UI는 control 노출만 검사하지 않는다. 대표값과 경계값에 대해
wire DTO 파싱, preview, apply, 새 session page와 query 결과까지 같은 조합을 검증한다.
...
재빌드와 검증을 위해 이 workspace에서 실행한 Viewer/Tauri 개발·Release 프로세스를 종료하는
작업도 별도 확인 없이 진행한다. 이 허용은 해당 프로젝트의 프로세스에만 적용하며, 다른 프로젝트나
사용자 작업과 관계된 프로세스는 종료하지 않는다.

## 사용자 확인 최소화

사용자가 구현이나 Phase 진행을 지시하면 그 범위 안의 코드 수정, fixture 생성, 일반 개발 의존성
설치, 테스트·성능 측정, 개발 서버와 브라우저·Tauri 실행, 프로젝트 Viewer/Tauri 프로세스 종료,
재빌드와 검증은 이미 승인된 것으로 보고 일일이 확인하지 않는다. 발견한 결함도 요청 범위 안이면
합리적인 판단으로 수정하고 계속 진행한다. 내부 단계 선택, 테스트 순서, subagent 역할 분담 때문에
사용자에게 선택이나 승인을 요구하지 않는다.

확인이 필요한 경우는 요청 범위를 실질적으로 바꾸는 선택, 보안 권한 확대, 사용자 데이터 삭제,
기술 스택 또는 네이티브 런타임 의존성 변경, 외부 배포처럼 새 권한이 반드시 필요한 작업으로
한정한다. 도구가 시스템 승인 창을 강제하는 경우를 제외하고, 이미 허용된 작업을 다시 묻지 않는다.

Subagent는 사용자에게 권한 또는 승인 요청을 직접 보내지 않는다. sandbox 밖 실행이나 새 권한이
필요해 보이면 승인 요청 도구를 호출하지 말고 가능한 범위를 계속 진행한 뒤 Root Agent에게만
차단 사유를 보고한다. Root Agent가 정말 불가피한 새 권한인지 판단하고, 기존 허용 범위로 해결할
수 있으면 사용자 확인 없이 처리한다.
